...
const accuWeather = 'http://dataservice.accuweather.com/forecasts/v1/';
...
const owmForecastEndPoint = 'https://api.openweathermap.org/data/2.5/forecast?';
...
export const fetchForecast = async (key) => {
  const query = `daily/5day/${key}?apikey=${accuKey}`;
  const forecastRequest = await fetch(`${accuWeather + query}`);
  const forecast = forecastRequest.json();

  return forecast;
};

export const fetchLocationKey = async (lat, lon) => {
  const query = `cities/geoposition/search?apikey=${accuKey}&q=${lat}%2C${lon}`;
  const locationKeyRequest = await fetch(`${accuWeather + query}`);
  const locationKey = locationKeyRequest.json();
